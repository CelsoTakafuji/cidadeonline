...
<div class="site-container">
    ...
    <section class="last_forcat">
		<!-- Exibe 4 registros das 3 subcategorias da categoria pai exibida no slide -->
        <h1>Por categoria!</h1>

        <?php
		$subCat = new Read;  
		$subCat->ExeRead("ws_categories", "WHERE category_parent = :category_parent ORDER BY category_name DESC LIMIT :limit OFFSET :offset", "category_parent={$cat}&limit=3&offset=0");
		if ($subCat->getResult()):
			$contsc = 0;
			foreach ($subCat->getResult() as $sc):
				$contsc++;
				echo '<section class="subcat'.$contsc.'">';
				echo '<h2 class="line_title"><span class="';
					echo ($contsc == 1) ? 'roxo' : (($contsc == 2) ? 'verde' : (($contsc == 3) ? 'azul' : 'red'));
				echo '">'.$sc['category_title'].'</span></h2>';
				
				$cat = Check::CatByName($sc['category_name']);
				$post->setPlaces("cat={$cat}&limit=1&offset=0");
				if (!$post->getResult()):
					WSErro("Desculpe, não existe um registro na categoria ".$sc['category_name']." para ser exibido agora. Favor, volte depois!", WS_INFOR);
				else:
					$new = $post->getResult()[0];
